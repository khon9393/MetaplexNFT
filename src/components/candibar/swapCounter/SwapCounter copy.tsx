  ...
  return (
    ...
    <div className="">
      ...
      {Object.entries(zodiacSigns).map(([sign, { icon, dateRange }]) => {
        ...
        if (isActive) {
        return (
            <div key={sign} className="p-4 bg-gray-800 border-l-2 border-gray-600 rounded-lg">
      ...
      <div className="flex-1 p-4 bg-gray-800 border-l-2 border-gray-600 rounded-lg w-full md:w-1/2">
      ...
      {Object.entries(zodiacSigns)
        ...
        <div key={sign} className="p-0 items-start flex justify-start mb-2">
          <p className={`text-sm md:text-base ${index === 0 ? 'text-white' : 'text-gray-500'}`}>{icon} {sign} NFT Collection {dateRange}</p>
        </div>
        ))}
      </div>
    </div>
