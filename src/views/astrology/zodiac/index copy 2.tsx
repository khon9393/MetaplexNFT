...
export const AstrologyZodiacView: FC = ({ }) => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 text-purple-500">
        {Object.entries(CandiZodiacSigns).map(([sign, { icon, dateRange }]) => (
          ...
          >
            <p className="flex text-3xl font-extrabold text-center">{icon} {sign}</p>
            <p className="flex text-center">{dateRange}</p>
          </div>
        ))}
      </div>
